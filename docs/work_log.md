...
## [main] デモ用のフォント拡大・フォントファミリー変更

### Step: フォントサイズ拡大 + Noto Sans JP / Roboto への統一
- **日時:** 2026-08-29
- **背景:** デモでの視認性向上のため、フォントサイズを目安10〜15%程度
  拡大し、あわせてフォントファミリーを日本語=Noto Sans JP、
  英語=Robotoに統一したいとの依頼があった。AskUserQuestionで
  「見出し（Space Grotesk）も含めて全体を変更するか」「拡大幅」を
  確認し、全体変更・+10〜15%程度で合意した。
- **実施内容:**
  - `frontend/src/style.css`の`--font-body`・`--font-display`を
    `"Roboto", "Noto Sans JP", ...`に統一（従来の"Zen Kaku Gothic New"・
    "Space Grotesk"を廃止）。Robotoにグリフがない和字は自動的に
    Noto Sans JPへフォールバックする仕組みを利用し、1つのフォント
    スタックで日英を切り替え不要にした。
  - `frontend/index.html`のGoogle Fonts読み込みを
    `Roboto:wght@400;500;700` + `Noto+Sans+JP:wght@400;500;700`に変更
    （JetBrains Monoは変更なし）。
  - `@media`ブロック内（モバイル・低高さビューポート向けの手動調整済み
    サイズ）を除く、通常フローの`font-size`宣言35箇所を、Pythonスクリプト
    で一括約1.12倍（端数は偶数pxに丸め）に拡大した。
- **確認結果:** `pixi run build`でビルド成功。Claude Browserで
  デスクトップ（ライト/ダーク）・英語表示・モバイル幅（375px）の
  4パターンを確認し、Roboto/Noto Sans JPが実際にロード・適用されている
  ことを`document.fonts`で確認、レイアウト崩れがないことを確認した。
- **サニティチェック:** ✅ ビルド成功、4パターンのブラウザ実機確認
- **コミットハッシュ:** `9d36c14`
- **副次的なミス:** フォントサイズ一括拡大に使ったPythonスクリプトが
  `Path.write_text()`のWindowsデフォルト改行変換により`style.css`を
  意図せずCRLFに変換していた（前回の`tests/`ファイルと同じ問題の再発）。
  バイト単位の`\r\n`→`\n`置換で修正した。以後、複数行を書き換える
  Pythonスクリプトは`write_bytes()`または明示的な改行指定を使うよう
  徹底する。

---

## [main] フォントサイズを5〜10%縮小（拡大しすぎの調整）

### Step: 直前の拡大が大きすぎたための微調整
- **日時:** 2026-08-29
- **背景:** 直前のフォント拡大（約+12%）が想定より大きかったとの
  フィードバックを受け、5〜10%の範囲で縮小したいとの依頼があった。
- **実施内容:** `frontend/src/style.css`の`@media`ブロック外（通常表示）の
  `font-size`宣言35箇所を、Pythonスクリプトで一律7.5%（5〜10%の中間）
  縮小した。前回CRLF化の副作用が起きた反省を踏まえ、今回は
  `Path.write_bytes()`で明示的にUTF-8バイト列として書き込み、
  改行コードがLFのまま保たれることを`file`コマンドで確認した。
- **確認結果:** `pixi run build`でビルド成功。Claude Browserで
  デスクトップ幅（1280px）のブラウザ実機確認を行い、アプリタイトルが
  24px→22px、ウェルカム見出しが34px→31px、本文ヒントが20px→18px
  になっていることを`getComputedStyle`で確認（いずれも5〜10%の
  範囲内）。768px未満のモバイル用メディアクエリの値は今回も対象外
  のため変化なし（想定通り）。
