...
export const UI_STRINGS: Record<Lang, Record<string, string>> = {
  ja: {
    ...
    mapAttribution: '建物: PLATEAU 3D都市モデル（整備時点）／背景地図: OpenStreetMap。整備後の新築建物は表示・検索対象に含まれません。',
    mapCloseBtn: '× 閉じる',
    ...
    welcomeAttribution: '※ 建物データは PLATEAU 3D都市モデル（整備時点）のため、整備後の新築建物は検索対象に含まれません。',
  ...
  en: {
    ...
    mapAttribution: 'Buildings: PLATEAU 3D City Model (as of data acquisition) / Basemap: OpenStreetMap. Buildings constructed after data acquisition are not shown or searchable.',
    mapCloseBtn: '× Close',
    ...
    welcomeAttribution: '* Building data is from the PLATEAU 3D City Model (as of data acquisition); buildings constructed afterward are not searchable.',
